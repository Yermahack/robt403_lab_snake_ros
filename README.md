...
Comparing these two graphs, we can see that current position is closer to goal position on end joint due to joint moving lower weight. However, the difference are not as significant as in the step response graph. Moreover, the unsmoothness of the goal position graph is caused by real time function, which we used to generate the sine function. 
